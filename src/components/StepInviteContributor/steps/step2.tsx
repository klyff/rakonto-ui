...
const Step2: React.FC<{ url: string }> = ({ url }) => {
  const [
        ...
        <Grid item xs={12} md={6}>
          <FieldArray name="emails">
            {({ remove, push }) => (
              <>
                <Typography>
                  Who would you like to invite to contribute? You can invite specific contributors via email, or you can
                  leave blank for open invitations to contribution
                </Typography>
                <Box sx={{ display: 'flex', flexFlow: 'column' }}>
                  ...
                  <TextField
                    ...
                    fullWidth
                    margin="dense"
                    placeholder="Type the name of contributor"
                    onBlur={e => {
                      nameOnBlur(e)
                      setEmailsTouched(true)
                    }}
